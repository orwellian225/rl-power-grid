from scripts.agents.DQN.Improvement_2.qrdqn import DQNAgent
from stable_baselines3.common.callbacks import BaseCallback
from scripts.agents.DQN.Improvement_2.dqn_env import Gym2OpEnv
import csv
import numpy as np
...
class SimpleMetricsCallback(BaseCallback):
    def __init__(self, log_dir, verbose=0):
        ...
        # Create metrics file
        self.metrics_file = open(f"{self.log_dir}/improvement_2_dqn_metrics.csv", mode='w', newline='')
...
env = Gym2OpEnv()
dqn_agent = DQNAgent(env)  # It is now using QR-DQN internally

# Initialize the custom callback
metrics_callback = SimpleMetricsCallback(log_dir)

# Train the QRDQN Agent with the callback
...
for run in range(num_runs):
    obs, _ = env.reset()
    dqn_agent.metrics.reset(24 * 60 // 5, f"improvement-2-dqn-agent-run-{run+1}")
    done = False
    total_reward = 0
    timesteps = 0
...
csv_filename = f"{log_dir}/improvement_2_dqn_evaluation_results.csv"
